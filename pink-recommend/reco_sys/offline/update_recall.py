...
# 5.---------hbase:cb_recall,hbase:history_recall
...
class UpdateRecall(SparkSessionBase):
    ...
    def update_als_recall(self):
        ...
        user_article_click = self.spark.sql("select * from user_article_basic"). \
            select(['user_id', 'post_id', 'clicked'])

        # 更换类型
        def change_types(row):
            return row.user_id, row.post_id, int(row.clicked)

        user_article_click = user_article_click.rdd.map(change_types).toDF(['user_id', 'post_id', 'clicked'])
        # 用户和文章ID超过ALS最大整数值，需要使用StringIndexer进行转换
        user_id_indexer = StringIndexer(inputCol='user_id', outputCol='als_user_id')
        post_id_indexer = StringIndexer(inputCol='post_id', outputCol='als_post_id')
        pip = Pipeline(stages=[user_id_indexer, post_id_indexer])
        pip_fit = pip.fit(user_article_click)
        als_user_article_click = pip_fit.transform(user_article_click)

        # 模型训练和推荐默认每个用户固定文章个数
        als = ALS(userCol='als_user_id', itemCol='als_post_id', ratingCol='clicked', checkpointInterval=1)
        ...
        refection_article = als_user_article_click.groupBy(['post_id']).max('als_post_id').withColumnRenamed(
            'max(als_post_id)', 'als_post_id')
        ...
        # +-----------+-------+----------------+
        # | als_user_id | user_id | als_post_id |
        # +-----------+-------+----------------+
        # | 8 | 2 | [163, 0.91328144] |
        # | 8 | 2 | [132, 0.91328144] |
        import pyspark.sql.functions as F
        recall_res = recall_res.withColumn('als_post_id', F.explode('recommendations')).drop('recommendations')
        ...
        # | 8 | 2 | 132 |
        def _post_id(row):
            return row.als_user_id, row.user_id, row.als_post_id[0]

        als_recall = recall_res.rdd.map(_post_id).toDF(['als_user_id', 'user_id', 'als_post_id'])
        als_recall = als_recall.join(refection_article, on=['als_post_id'], how='left').select(
            ['user_id', 'post_id'])
        # 得到每个用户ID 对应推荐文章
        # +-------------------+----------+
        # | user_id | post_id |
        ...
        # | 1 | 44075 |
        # 分组统计每个用户，推荐列表
        # als_recall = als_recall.groupby('user_id').agg(F.collect_list('post_id')).withColumnRenamed(
        #     'collect_list(post_id)', 'article_list')
        self.spark.sql("use pink")
        news_article_basic = self.spark.sql("select post_id, category_id from posts")
        als_recall = als_recall.join(news_article_basic, on=['post_id'], how='left')
        als_recall = als_recall.groupBy(['user_id', 'category_id']).agg(F.collect_list('post_id')).withColumnRenamed(
            'collect_list(post_id)', 'article_list')
            ...
            for row in partition:
                with pool.connection() as conn:
                    ...
                    data = history_table.cells('reco:his:{}'.format(row.user_id).encode(),
                                               'category:{}'.format(row.category_id).encode())
                    ...
                    if reco_res:
                        ...
                        table.put('recall:user:{}'.format(row.user_id).encode(),
                                  {'als:{}'.format(row.category_id).encode(): str(reco_res).encode()})

                        # 放入历史推荐过文章
                        history_table.put("reco:his:{}".format(row.user_id).encode(),
                                          {'category:{}'.format(row.category_id): str(reco_res).encode()})
                    conn.close()
    ...
    def update_content_recall(self):
        ...
        def save_content_filter_history_to__recall(partition):
            ...
            with pool.connection() as conn:
                ...
                # key:   post_id,    column:  similar:post_id
                similar_table = conn.table('article_similar')
                # 循环partition
                for row in partition:
                    # 获取相似文章结果表
                    similar_article = similar_table.row(str(row.post_id).encode(),
                                                        columns=[b'similar'])
                    ...
                    if _srt:
                        ...
                        if reco_res:
                            ...
                            content_table.put("recall:user:{}".format(row.user_id).encode(),
                                              {'content:{}'.format(row.category_id).encode(): str(reco_res).encode()})

                            # 放入历史推荐过文章
                            history_table.put("reco:his:{}".format(row.user_id).encode(),
                                              {'category:{}'.format(row.category_id).encode(): str(reco_res).encode()})
